...
export function HeroCrew({ data, current }) {
  ...
  return (
    <motion.div key={current} className="text-center border-b mx-6 border-borderColor mb-9 md:border-none md:mb-0">
      {
        data 
          ? <motion.img 
              className="mx-auto h-[222px] md:h-[532px] desktop:h-[712px]" 
              src={images[current]} 
              alt={`photo of ${data.name}`} 
              variants={imgVariants}
              initial="initial"
...
export function HeroTechnology({ data, current }) {
  ...
  return (
    <div className="text-center mb-9 block w-full md:mb-14 overflow-hidden">
      {
        data 
          ? <picture className=' overflow-hidden'>
              ...
              <motion.img 
                className="w-full min-h-[170px] md:h-[310px] lg:w-[unset] lg:h-[unset] lg:ml-auto" 
                src={images[current]?.landscape} 
                alt={`photo of ${data.name}`} 
                key={current}
                variants={imgVariants}
                initial="initial"
